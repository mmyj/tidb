...
func (s *testSuite7) TestSlidingWindowFunctions(c *C) {
	...
	result.Check(testkit.Rows("M 0", "F 0", "F 0", "F 0", "M 0", "<nil> 0", "<nil> 0"))
	result = tk.MustQuery("SELECT sex, COUNT(id) OVER (PARTITION BY sex ORDER BY id RANGE BETWEEN 1 FOLLOWING and 2 FOLLOWING) FROM t ORDER BY id;")
	result.Check(testkit.Rows("M 0", "F 2", "F 1", "F 0", "M 0", "<nil> 1", "<nil> 0"))
	...
	result.Check(testkit.Rows("M 0", "F 0", "F 0", "F 0", "M 0", "<nil> 0", "<nil> 0"))
	result = tk.MustQuery("SELECT sex, COUNT(id) OVER (PARTITION BY sex ORDER BY id RANGE BETWEEN 2 PRECEDING and 1 PRECEDING) FROM t ORDER BY id;")
	result.Check(testkit.Rows("M 0", "F 0", "F 1", "F 2", "M 0", "<nil> 0", "<nil> 1"))
}
